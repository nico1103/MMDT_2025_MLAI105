...
def plot_predictions_comparison(y_actual_test, y_pred_initial, y_pred_improved, fname):
    import matplotlib.pyplot as plt
    plt.figure(figsize=(10, 6))
    
    # Initial model
    plt.scatter(y_actual_test, y_pred_initial, color='blue', alpha=0.6, label='Initial Model')
    
    # Improved model
    plt.scatter(y_actual_test, y_pred_improved, color='red', alpha=0.6, label='Improved Model')
    
    plt.xlabel('Actual Value')
    plt.ylabel('Predicted Value')
    plt.title('Predicted vs Actual: Initial vs Improved Model')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
...
def plot_loss_comparison(history_initial, history_improved, fname):
    import matplotlib.pyplot as plt
    plt.figure(figsize=(12, 6))
    
    # Initial Model
    plt.plot(history_initial.history['loss'], label='Initial Model - Train Loss', color='blue')
    plt.plot(history_initial.history['val_loss'], label='Initial Model - Val Loss', linestyle='dashed', color='blue')
    
    # Improved Model
    plt.plot(history_improved.history['loss'], label='Improved Model - Train Loss', color='red')
    plt.plot(history_improved.history['val_loss'], label='Improved Model - Val Loss', linestyle='dashed', color='red')
    
    plt.title('Initial vs Improved Model - Training and Validation Loss')
    plt.xlabel('Epochs')
